        ...
        if item.name == name:  # node found
            ...
            if "selector" in name:
                if item.ticked is False:
                    ...
                    for child_node in children:
                        for obj in tree_data:
                            if obj.name == child_node:   # child found
                                if obj.ticked is True:
                                    tree_data = tick_node(tree_data, obj.name)
                                    nodes_ticked.append(obj.name)
                                    break

            elif "sequence" in item.name:
                running = False
                children = item.children
                ...
                if running is False:
                    ...
                    if "selector" in item.parent:
                        tree_data = lookup_parent_selector(tree_data, item.name)
                    break

            elif "parallel" in item.name:
                running = False
                children = item.children
                for child_node in children:
                    for obj in tree_data:
...
def get_database_values(host_name, user_name, pas, database_name, table_name):
...
def get_database_column(column):
    ...
    con = pymysql.connect(host=host, user=user, password=password, database=database)  # MySQL database
    data = []
...
def big_traverse(tree_data):
    #get_database_values("localhost", "root", "", "delivery tracking", "packages")
    r = 0
    run = True
    while run is True:
        r += 1
...
def run_bot(tree_data, trigger_tag):
    ...
    while bot_service:
        ...
        if user_input == "quit":
            ...
            if tag == trigger_tag:
                ...
                tree_data = big_traverse(tree_data)
                print(" ")
                Bot.feedback(intents)

            else:
                bot_service = Bot.activate_service(user_input, tag, intents, trigger_tag)
                if bot_service == trigger_tag:
                    Bot.write_to_file("Bot: Tracking...\n")
                    tree_data = big_traverse(tree_data)
                    print(" ")
                    Bot.feedback(intents)
...
def progress_window(nodes, ticked):
    window = Tk()
